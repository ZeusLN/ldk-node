...
#[cfg(not(feature = "uniffi"))]
use bitcoin::FeeRate;
use bitcoin::{Address, Amount, BlockHash, Network, WPubkeyHash};
#[cfg(feature = "uniffi")]
...
impl Node {
	...
	///
	/// Returns the possible counterparty force-close script pubkeys derivable from the node's
	/// seed, paired with the corresponding signing keys. Intended for offline construction of
	/// sweep transactions when you already have a specific commitment txid in hand.
	pub fn get_counterparty_close_scripts_and_keys(
		&self,
	) -> Vec<(bitcoin::ScriptBuf, bitcoin::secp256k1::SecretKey)> {
